...
export default function CoverLetterTemplateEditScreen() {
  ...
  const template = useAsyncData(
    () => (isNew ? Promise.resolve({ success: true as const, data: null }) : coverLetterService.getTemplate(id!)),
    [id]
  );
  ...
  const saveAction = useAsyncAction(async () => {
    if (isNew) {
      return coverLetterService.createTemplate({ name, description, body });
    }
    return coverLetterService.updateTemplate(id!, { name, description, body });
  });

  const handleSave = async () => {
    if (!name.trim() || !body.trim()) {
      Alert.alert('Missing fields', 'Name and body are required.');
      return;
    }
      ...
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <ScrollView contentContainerStyle={styles.scrollContent} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.form}>
            <Input label="Name" required value={name} onChangeText={setName} placeholder="Template name" />
            <Input label="Description" value={description} onChangeText={setDescription} placeholder="Brief description" />
            <TextArea label="Body" required value={body} onChangeText={setBody} placeholder="Letter content…" />
            ...
            <Card style={styles.hints}>
              ...
              <Text style={[styles.hintsText, { color: colors.textMuted }]}>
                {PLACEHOLDER_HINTS.filter((p) => p !== '{{candidate_name}}').join(' · ')}
              </Text>
            </Card>
          </View>
